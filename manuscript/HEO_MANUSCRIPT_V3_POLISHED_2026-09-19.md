...
The distinction is difficult to resolve when GITT is reduced to a single apparent diffusion coefficient. Conventional GITT-derived diffusivity is sensitive to geometry, equilibrium assumptions, time-window selection, and the validity of the underlying diffusion model,[15] which become nontrivial in a structurally reconstructive conversion electrode. Current-interruption and extended GITT analyses have shown that the transient can contain separable kinetic information beyond one apparent diffusivity,[16,23,24] and phase-transformation GITT has been used to infer interface mobility or compare transformation kinetics in two-phase electrodes.[21,22] Structural relaxation after current interruption has also been observed directly,[26] while phase-field studies show that non-Fickian phase evolution can alter GITT/PITT interpretation.[27,28] These studies provide the methodological basis for treating the present relaxation response as more than a diffusion coefficient; they are not the focus of this work.

The central question is instead **how ball milling and Mg incorporation alter the electrochemical use and phase evolution of the same spinel HEO system**. This study compares HEO, BM-HEO, Mg-HEO, and BM-Mg-HEO using structural characterization, galvanostatic performance, direct current-off analysis of 10 min GITT pulses followed by 60 min relaxation, and a reduced spatial model. The analysis separates accessible capacity, polarization amplitude, transition width, and relaxation time so that the different consequences of the two synthesis variables can be identified. The resulting 2 × 2 comparison shows that ball milling primarily increases accessibility and redistributes the phase-transforming reaction, whereas Mg primarily reduces transformation extent through structural stabilization.
...
The voltage/state localization links this excess response to structural conversion. Atomic-scale work has established extensive reconstruction of five-cation spinel HEOs during lithiation,[5] and Jin et al. directly resolved a spinel → mixed spinel/rock-salt → rock-salt pathway in the same five-cation family.[7] Phase-transforming battery materials can exhibit additional nucleation- and phase-boundary-related polarization,[17] and structural relaxation can continue after current interruption.[26] The present late-stage hump is therefore assigned primarily to the spinel-to-rock-salt/conversion transformation, while its magnitude is treated as an electrochemical signature of the transformation rather than a direct phase-fraction measurement.
...
The reduced spatial model therefore tests a minimum working set of independent physical coordinates needed to rationalize the HEO trends in Figure 5a. Transport, transformation stabilization/extent, and local-transition/structural-mobility heterogeneity are allowed to vary independently. Mg requires stabilization of the parent/intermediate state together with slower residual structural mobility. BM requires a distribution of local transition conditions and structural mobilities rather than a uniform acceleration of transport. The parameters are treated as effective, non-unique coordinates constrained by the observed HEO trends rather than as uniquely extracted material constants.
