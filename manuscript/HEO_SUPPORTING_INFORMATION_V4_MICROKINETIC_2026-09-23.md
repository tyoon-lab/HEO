...
# S7. Minimal conversion microkinetic model and robustness

The microkinetic model is used only to test whether more accessible conversion can coexist with slower post-interruption relaxation in a coupled multi-step conversion network. It is not fitted to assign unique microscopic rate constants or a unique rate-determining step.

## S7.1. Effective reaction network
...
$A$, $B$, $N$, and $P$ are effective kinetic states. The first and third reactions carry Faradaic current, and the second represents an effective nucleation/activity coordinate.

A minimal implementation uses the state vector $(b,p,n)$ with $a=1-b-p$. Butler–Volmer-like forward/reverse electrochemical rates are used for the first and third reactions, while the internal coordinate follows a kinetic evolution law. The implementation is retained in the repository file \`modeling/heo_conversion_microkinetics_electrochemical_growth.py\`.

## S7.2. Current-on and current-off constraints

During the galvanostatic pulse,

\[
j_{\mathrm{ext}}=r_1+r_3.
\]

During the 3600 s current-off period,

\[
j_{\mathrm{ext}}=r_1+r_3=0.
\]

The open-circuit condition does not require each internal Faradaic pathway to vanish independently. A representative simulation gives opposing finite rates immediately after interruption, approximately $r_1=-5.0\times10^{-5}$ and $r_3=+5.0\times10^{-5}$ in model units, while $r_1+r_3$ remains at numerical zero. The internal counter-current then decays during the rest period together with the voltage relaxation.

The compact diagnostic output is stored as \`modeling/HEO_MICROKINETIC_CURRENT_OFF_BALANCE_2026-09-23.csv\`.

**Figure S21.** Representative current-off internal counter-current and voltage-relaxation trajectory.

## S7.3. Local linearized interpretation

Near a local cycled state,
...
The coefficients $B_i$ depend on the state excitation/population and voltage sensitivity, whereas $\tau_i$ values arise from the eigenvalues of the kinetic Jacobian. This formal separation is the basis for interpreting capacity/reaction accessibility and current-off relaxation as related but non-identical kinetic consequences.

The model quantities are not equated with experimentally measured microscopic eigenmodes.

## S7.4. Synthetic current-dependence discrimination

Two illustrative parameter regimes were constructed:
- nucleation-dominant;
- electrochemical-growth-dominant.

At the reference pulse current they were intentionally matched to produce nearly identical current-off descriptors:

| regime | residual relaxation at 3 s | $t_{63}$ |
|---|---:|---:|
| nucleation-dominant | 24.82 mV | 13.66 min |
| electrochemical-growth-dominant | 24.13 mV | 13.70 min |

When the relative pulse current is changed, the regimes separate, especially in $t_{63}$. This result is retained only as an identifiability/future-discrimination analysis; it is not used to argue that additional current-dependent GITT is required for the present manuscript.

The numerical sweep is stored as \`modeling/HEO_MICROKINETIC_CURRENT_SWEEP_2026-09-23.csv\`.

**Figure S22.** Synthetic pulse-current dependence of the two illustrative microkinetic regimes.

## S7.5. Interpretation boundaries

The model does not uniquely determine:
- microscopic identities of $A$, $B$, $N$, or $P$;
- unique elementary rate constants;
...
The model supports mechanistic consistency: reaction accessibility/state population and internal relaxation eigen-timescales can vary sufficiently independently for greater accessible conversion to coexist with slower ensemble relaxation.

**Table S7.** Representative model parameters, matched-regime descriptors, current-sweep output, and current-off current-balance diagnostics.
...
- Figure S21: representative microkinetic current-off internal counter-current and voltage relaxation.
- Figure S22: synthetic pulse-current discrimination of illustrative microkinetic regimes.
...
- Table S7: microkinetic parameters, current-off current balance, and synthetic current-sweep descriptors.
