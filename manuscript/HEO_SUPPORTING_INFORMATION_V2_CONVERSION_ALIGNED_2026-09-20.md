...
The GITT signal itself does not uniquely separate metal/Li2O nucleation, cation/oxygen rearrangement, phase-boundary motion, strain, or other microscopic contributions. The late-stage feature is described as conversion/transformation-associated because its peak voltage coincides with the first-cycle cathodic differential-capacity feature across all four samples and because independent literature on the same HEO family directly identifies low-voltage conversion chemistry.

---

## S4.3. Cross-check against the first-cycle cathodic differential-capacity feature

The latest first-cycle voltage profiles from the 2026-09-17 HEO progress presentation were stored as vector graphics and were reconstructed at high resolution. The reconstructed terminal capacities agree with the plotted first-cycle values to within approximately 0.03%.
...
All four peak pairs are localized within 32 mV. The dQ/dV peak locations are also insensitive to reasonable smoothing-window changes: HEO 0.544–0.547 V, BM-HEO ~0.589 V, Mg-HEO 0.408–0.419 V, and BM-Mg-HEO 0.485–0.486 V.
...
**Data-source boundary.** The numerical continuous-GCD source files corresponding to these latest profiles are not currently available. The present values were reconstructed from the user's own vector voltage-profile plots, not from raster digitization. The original numerical profiles should replace this source if recovered before submission.

**Figure S18a.** Peak-voltage comparison between the first-cycle cathodic differential-capacity feature and GITT excess relaxation.
