...
Lithiation of these spinel HEOs is not adequately described as diffusion through a structurally invariant host. Atomic-scale analysis of the same five-cation spinel family showed Mn nanocrystal formation already at 0.5 V lithiation and metallic Cr, Fe, Ni, and Co at deeper lithiation,[5] directly demonstrating conversion chemistry in the low-voltage region. Complementary in-situ XRD/ex-situ TEM work resolved a spinel → mixed spinel/rock-salt → rock-salt pathway,[7] while recent studies describe a rock-salt-like intermediate coexisting with Li₂O and metallic alloy phases and identify oxygen migration as an important kinetic coordinate of conversion.[20] The low-voltage reaction is therefore better viewed as conversion-associated structural evolution than as a single crystallographic phase transition. Nucleation, cation/oxygen rearrangement, phase-boundary motion, strain accommodation, and heterogeneous conversion can all contribute to the measured polarization and relaxation.

Ball milling and Mg incorporation provide two physically distinct perturbations of this reconstructive reaction. In the same five-cation HEO family, ball-milled comparisons and conventional GITT analysis have already shown that mechanical processing strongly changes electrochemical response.[6] High-energy milling can perturb the spinel/rock-salt balance,[11] while particle fragmentation increases conversion reversibility and interfacial storage during cycling.[12] Morphology likewise changes rate and cycling behavior in the same composition space.[13] Mg introduces a different constraint. Mg-containing conversion HEO studies show that electrochemically inactive Mg-derived components can stabilize oxide-derived structures,[8–10] and operando imaging has linked higher Mg content with improved structural retention but lower accessible capacity.[9] Mechanical processing and Mg incorporation therefore need not act on the same physical coordinate of conversion.

The distinction is difficult to resolve when GITT is reduced to a single apparent diffusion coefficient. Conventional GITT-derived diffusivity is sensitive to geometry, equilibrium assumptions, time-window selection, and the validity of the underlying diffusion model,[15] all of which become nontrivial in a reconstructive conversion electrode. The current-off transient contains additional information. Early current-interruption behavior can be analyzed through an E–√t representation under appropriate short-time conditions,[16] while the longer relaxation amplitude and characteristic time can be quantified without forcing the entire transient into one diffusivity. Phase-transforming electrodes can exhibit substantial nucleation-related overpotential,[17] and phase-field theory shows that nucleation and phase-separation energetics depend strongly on particle/domain size and surface contribution.[18,19]

The unresolved question is therefore not whether ball milling or Mg changes electrochemical performance, but **which physical coordinate each synthesis variable changes**. This study compares HEO, BM-HEO, Mg-HEO, and BM-Mg-HEO using structural characterization, galvanostatic performance, direct current-off analysis of 10 min GITT pulses followed by 60 min relaxation, and a reduced spatial mechanism-sufficiency model. The analysis separates accessible capacity, polarization amplitude, conversion-response width, and relaxation time rather than collapsing them into one apparent transport parameter. The resulting 2 × 2 comparison shows that ball milling primarily increases accessibility and redistributes conversion over a broader reaction-state interval, whereas Mg suppresses and delays accessible conversion through stabilization of the oxide-derived parent/intermediate state.
...
BM-Mg-HEO provides an internal control. Milling increases accessible interface and capacity within the Mg-containing composition and partially restores the conversion-associated response toward higher potential and larger width, but it does not recover the concentrated HEO-like response. Mechanical processing can therefore reopen accessible conversion without removing the stronger compositional stabilization imposed by Mg.
