...
Structural and chemical modifications in conversion-type high-entropy oxide (HEO) anodes are commonly identified by materials characterization and conventional electrochemical performance, but their state-resolved electrochemical consequences remain difficult to separate. Here, spinel Fe–Co–Ni–Cr–Mn HEO, ball-milled HEO (BM-HEO), Mg-containing HEO (Mg-HEO), and BM-Mg-HEO are used to examine how known materials-level perturbations appear in conversion and relaxation behavior. Ball milling increases accessible capacity and BET surface area from 3.94 to 18.159 m² g⁻¹, yet the model-free current-off relaxation time, $t_{63}$, lengthens from 8.68 to 11.57 min. Mg-HEO shows lower accessible capacity and a substantially smaller relaxation amplitude, while $t_{63}$ increases to 11.01 min rather than becoming faster. A late-stage GITT excess response is quantified by its peak position, amplitude, width, and area; its peak voltage matches the independently derived first-cycle cathodic dQ/dV feature within 32 mV for all four materials, linking the transient response to conversion electrochemistry. Milling lowers and broadens this conversion-associated response, whereas Mg strongly suppresses it and shifts it to lower potential. A reduced spatial model shows that these experimentally observed combinations are compatible with separate structural stabilization and relaxation effects. The results demonstrate how known HEO structural modifications can be translated into state-resolved electrochemical descriptors rather than a single apparent diffusion coefficient.
