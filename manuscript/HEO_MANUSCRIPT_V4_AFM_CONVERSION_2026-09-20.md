...
The model couples a conserved Li-state variable, $c(r,t)$, with a nonconserved internal variable, $\phi(r,t)$, that represents progression of a local region from an oxide-derived parent/intermediate state toward a more deeply converted state. The variable $\phi$ is used only as an effective description of conversion-associated structural evolution; it is not identified with a measured rock-salt fraction, metallic fraction, Li₂O fraction, or any other unique microscopic phase fraction. Likewise, the model lithiation variable is not calibrated directly to experimental normalized capacity or voltage.
