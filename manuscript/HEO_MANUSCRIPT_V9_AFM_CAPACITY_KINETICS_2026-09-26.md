# HEO Manuscript v9 — AFM-Target Capacity–Kinetics / GITT Ranking Draft

**Date:** 2026-09-26  
**Status:** toolkit-aligned observable-contradiction architecture; BM capacity–relaxation mismatch is the primary scientific event, GITT apparent-diffusivity inversion is the independent kinetic-ranking test, and microkinetics is retained as a bounded existence proof  
...
Under finite-rate operation, faster reaction kinetics are generally expected to increase the accessible capacity of conversion-type anodes. Conversion, however, proceeds through multiple coupled processes, including electron transfer, structural reconstruction, nucleation and phase growth, and cation/oxygen redistribution, raising the question of whether accessible capacity and kinetic response necessarily evolve together. Here, spinel high-entropy oxide (HEO) anodes are used as a model system to examine this relationship, with ball milling and Mg incorporation providing complementary perturbations that respectively enhance and suppress accessible conversion capacity. Ball milling reveals that higher accessible capacity can coexist with slower relaxation, while conventional GITT analysis gives the opposite kinetic ranking. Across 37 state-matched points, the apparent diffusivity is higher for BM-HEO at every point, whereas the direct current-off relaxation is slower at 35 points. Mg incorporation provides a complementary constraint by suppressing accessible conversion and the relaxation amplitude without accelerating the relaxation. Microkinetic analysis shows that this behavior can arise naturally from the multistep character of conversion. Capacity, apparent diffusivity, and relaxation therefore cannot be reduced to a single fast–slow kinetic coordinate, and apparent GITT diffusivity should not be used alone to rank overall conversion kinetics.
...
Ball milling reveals that higher accessible capacity can coexist with slower relaxation in spinel HEO conversion anodes. Over the common first-lithiation interval, BM-HEO accesses more capacity than HEO while its effective current-off relaxation is slower. Mg incorporation provides a complementary constraint by suppressing accessible conversion and the relaxation amplitude without accelerating the relaxation. Differential-capacity analysis localizes the anomalous relaxation to the conversion region, and cycle-resolved GITT shows that the capacity–relaxation mismatch is not explained by first-cycle irreversibility alone.
