# HEO Manuscript v11 — AFM-Target Capacity–Kinetics / GITT Ranking Draft

**Date:** 2026-09-26  
**Status:** post-audit revision of v10; scientific architecture preserved, observation-before-localization logic tightened, model protocols clarified, and known submission blockers kept explicit  
**Scientific backbone:** Figures 1–2 materials perturbations → Figure 3 accessible capacity → Figure 4 BM capacity–relaxation mismatch + conventional-GITT ranking inversion → Figure 5 conversion localization → Figure 6 history dependence → Figure 7 microkinetic existence proof  
...
Under finite-rate operation, faster reaction kinetics are generally expected to increase the accessible capacity of conversion-type anodes. Conversion, however, proceeds through multiple coupled processes, including electron transfer, structural reconstruction, nucleation and phase growth, and cation/oxygen redistribution, raising the question of whether accessible capacity and kinetic response necessarily evolve together. Here, spinel high-entropy oxide (HEO) anodes are used as a model system to examine this relationship, with ball milling and Mg incorporation providing complementary perturbations that respectively enhance and suppress accessible conversion capacity. Ball milling reveals that higher accessible capacity can coexist with slower conversion-associated kinetics, while conventional GITT analysis gives the opposite kinetic ranking. Across 37 state-matched points, the apparent diffusivity is higher for BM-HEO at every point, whereas the direct current-off relaxation is slower at 35 points. Mg incorporation provides a complementary constraint by suppressing accessible conversion and the relaxation amplitude while the effective conversion-associated relaxation remains slower rather than faster. Microkinetic analysis shows that such non-monotonic combinations of reaction extent, relaxation amplitude, and relaxation timescale are physically admissible within a multistep conversion network. Capacity, apparent diffusivity, and relaxation therefore cannot be reduced to a single fast–slow kinetic coordinate, and apparent GITT diffusivity should not be used alone to rank overall conversion kinetics.

**Keywords:** high-entropy oxide; conversion anode; ball milling; magnesium incorporation; GITT; apparent diffusion coefficient; electrochemical relaxation; microkinetics

# 1. Introduction

Under finite-rate operation, faster reaction kinetics are generally expected to reduce polarization and increase the fraction of thermodynamically available capacity reached before the voltage cutoff. Accessible capacity is therefore often interpreted together with kinetic descriptors when comparing electrode modifications. For a simple homogeneous reaction, the same kinetic acceleration should also shorten the relaxation after current interruption. Whether these observables preserve a common fast–slow ordering is less obvious when the electrode reaction is reconstructive.

Conversion reactions combine electron transfer and Li incorporation with metal–oxygen bond rearrangement, nucleation and phase growth, cation/oxygen redistribution, and product formation across heterogeneous regions of the electrode. These processes can operate on different timescales and need not respond uniformly to structural or compositional modification. A material change may therefore alter the amount of reaction accessed under load and the subsequent relaxation dynamics by different amounts.

Conventional GITT-derived apparent Li-ion diffusion coefficients are nevertheless widely used as kinetic comparators in conversion-type anodes, including HEOs.[6,33–36] A larger apparent $D_{\mathrm{Li}}$ is commonly associated with faster ion transport and improved rate or capacity performance. The conventional reduction compresses the pulse response into one transport parameter, although finite reaction kinetics can distort GITT-derived diffusion coefficients,[37] and phase-transformation, non-ideal thermodynamic, and pulse-model effects can require treatment beyond a standard single-$D$ expression.[15,21,24] Recent mechanistic work on spinel HEOs likewise distinguishes insertion-stage Li transport from conversion-stage reaction, interface, and oxygen-migration processes.[20] Whether apparent GITT diffusivity preserves the actual fast–slow ordering of a reconstructive conversion electrode therefore requires an independent kinetic test.

High-entropy oxides provide a useful material system for such a test. Spinel (FeCoNiCrMn)₃O₄ is a conversion-type HEO anode in which multiple redox-active cations share a common oxide structure.[1–4,6] Lithiation produces substantial reconstruction: metallic species emerge progressively,[5] while diffraction and microscopy studies report spinel-to-rock-salt-like evolution, Li₂O/metal formation, and oxygen/cation rearrangement.[7,20] Ball milling and Mg incorporation provide complementary perturbations of this conversion landscape. Milling increases surface area and has been associated with enhanced conversion reversibility,[6,12] whereas Mg-containing HEOs show greater structural retention but lower accessible capacity.[8–10]

Here, HEO, ball-milled HEO (BM-HEO), Mg-containing HEO (Mg-HEO), and BM-Mg-HEO are used to test the relation among accessible capacity, conventional GITT apparent diffusivity, and current-off relaxation. The central experimental question is whether the larger capacity produced by ball milling corresponds to faster conversion-associated kinetics. A state-matched conventional-GITT analysis provides an independent test of whether apparent diffusivity gives the same kinetic ordering as the directly observed relaxation. Differential-capacity analysis then determines whether the anomalous relaxation is localized to conversion, cycle-resolved GITT tests whether the mismatch persists with reaction history, and a coarse-grained microkinetic model evaluates whether the observed behavior can arise from a multistep conversion network.
...
The central test is therefore set by ball milling: does its larger accessible capacity correspond to faster conversion-associated kinetics? Mg incorporation provides a complementary perturbation that changes accessible conversion in the opposite direction. Figure 4 compares these capacity changes with the state-resolved GITT relaxation and then asks whether conventional apparent diffusivity gives the same kinetic ordering.

## 2.3. Ball milling reveals a capacity–kinetics mismatch and an inverted apparent-diffusivity ranking
...
Ball milling provides the primary contradiction to the conventional capacity–kinetics expectation. Over the common 200–800 mAh g⁻¹ interval, the median $t_{63}$ increases from 8.68 min for HEO to 11.57 min for BM-HEO even though ball milling increases the first-cycle accessible capacity. The same direction is present in the Mg-containing pair, where the median $t_{63}$ increases from 11.01 min for Mg-HEO to 12.99 min for BM-Mg-HEO. Higher accessible capacity therefore coexists with slower effective relaxation after milling.

A conventional GITT diffusivity analysis of the compositionally identical HEO/BM-HEO pair gives the opposite kinetic ordering (Figure 4b). Across 37 state-matched points spanning 200–800 mAh g⁻¹, $D_{\mathrm{app,BM}}/D_{\mathrm{app,HEO}}$ remains above unity at every point and has a median value of 1.78. In contrast, the direct relaxation-rate ratio, $t_{63,\mathrm{HEO}}/t_{63,\mathrm{BM}}$, has a median value of 0.762 and lies below unity at 35 of 37 points. Conventional $D_{\mathrm{app}}$ therefore classifies BM-HEO as faster over the same state range in which the current-off relaxation is predominantly slower.
...
Mg incorporation provides a complementary constraint rather than a second headline contradiction. The median $\Delta E_{\mathrm{relax}}$ decreases from 160.9 mV for HEO to 109.5 mV for Mg-HEO, while $t_{63}$ increases from 8.68 to 11.01 min. Thus, the smaller nonequilibrium voltage response after Mg incorporation is not accompanied by faster relaxation. Capacity, response magnitude, and relaxation timescale do not collapse onto one scalar kinetic coordinate across the four materials.
...
This localization ties the excess current-off response to the conversion region. Together with the longer $t_{63}$ in BM-HEO, it shows that the larger accessible capacity after milling is not accompanied by uniformly faster conversion-region relaxation. The excess feature is also lower and broader after milling, indicating that the conversion-associated nonequilibrium response is redistributed rather than simply amplified. The higher capacity after milling therefore cannot be described as uniform acceleration of a single conversion-rate coordinate.
...
Figure 5 therefore establishes that the excess relaxation feature in the state-resolved GITT response coincides with the conversion region rather than with a generic relaxation background. The next question is whether the capacity–relaxation relation is confined to the first conversion or persists as the electrode evolves with cycling.
...
An illustrative heterogeneous calculation then changes reaction accessibility together with one internal timescale. The reference fast population is retained while an additional population with a slower reconstruction step becomes electrochemically accessible (Figure 7d). The cutoff capacity increases from 0.558 to 0.657 (+17.7%), while matched-state $t_{63}$ increases from 13.45 to 15.28 min (+13.6%). Higher accessible capacity and slower relaxation can therefore coexist within the same multistep kinetic network.
...
To audit the conventional diffusivity interpretation, a state-matched relative $D_{\mathrm{GITT,app}}$ comparison was performed for the compositionally identical HEO/BM-HEO pair over 200–800 mAh g⁻¹. The conventional finite-pulse form $D\propto (m_B/S)^2(\Delta E_s/\Delta E_\tau)^2/\tau$ was used only for relative ranking. $\Delta E_s$ was the relaxed voltage increment between consecutive rest endpoints, and $\Delta E_\tau$ was the voltage excursion from 3 s after pulse onset to the pulse end. The same 3 s reference suppresses the unresolved initial fast/ohmic contribution. The current relative comparison assumes the same geometric electrode area for HEO and BM-HEO. Absolute $D$ values are not used because the final electrode-area and molar-volume metadata are not yet frozen; the electrode-area metadata must be verified before submission, and the full relative-analysis definition and sensitivity audit are given in the Supporting Information.
...
**Figure 4. Ball milling reveals a capacity–kinetics mismatch and an inverted apparent-diffusivity ranking.** (a) Representative 10 min GITT pulse followed by a 60 min open-circuit rest, defining the 3 s-to-60 min relaxation magnitude, $\Delta E_{\mathrm{relax}}$, and model-free effective kinetic timescale, $t_{63}$. (b) State-matched HEO/BM-HEO comparison of the conventional apparent-diffusivity ratio, $D_{\mathrm{app,BM}}/D_{\mathrm{app,HEO}}$, and the direct relaxation-rate ratio, $t_{63,\mathrm{HEO}}/t_{63,\mathrm{BM}}$; both are oriented so values above unity correspond to faster BM-HEO. The conventional $D_{\mathrm{app}}$ ratio is above unity at all 37 matched states, whereas the relaxation-rate ratio is below unity at 35 of 37 states. (c) Median $t_{63}$ over the common 200–800 mAh g⁻¹ interval for all four materials. (d) First-cycle reversible capacity versus median $t_{63}$. Ball milling increases accessible capacity while lengthening the effective relaxation, and the conventional apparent diffusivity gives the opposite kinetic ranking for the HEO/BM-HEO pair.

**Figure 5. The excess current-off relaxation is localized to the conversion region.** (a) State-resolved first-cycle relaxation showing the conversion-associated excess feature. (b) Background-subtracted GITT excess relaxation on a voltage axis together with the first-cycle cathodic $dQ/dV$ response. (c) Comparison of $dQ/dV$ and GITT excess-peak voltages; all pairs lie within 32 mV. (d) Peak amplitude versus FWHM-like width of the excess response. The peak correspondence associates the excess current-off response with conversion rather than with a generic cell-relaxation background.
...
**Figure 7. Microkinetic analysis shows how the capacity–kinetics mismatch can arise from multistep conversion.** (a) Literature-informed coarse-grained conversion network, $O\rightleftharpoons I\rightleftharpoons I^*\rightleftharpoons C$, containing two reversible Faradaic steps separated by a structural reconstruction/conversion-activation step. (b) Current-on and current-off balances showing that zero external current can coexist with finite internal redistribution at conserved overall lithiation. (c) Homogeneous kinetic-speed control: scaling all rates together increases cutoff-limited capacity and shortens matched-state $t_{63}$. (d) Heterogeneous-accessibility existence proof: retaining the original fast population while adding an accessible slower-reconstructing population produces both higher cutoff capacity and longer matched-state $t_{63}$. The calculation is an existence proof showing that higher accessible capacity and slower relaxation can coexist within a multistep kinetic network without assigning a unique microscopic mechanism to ball milling.
