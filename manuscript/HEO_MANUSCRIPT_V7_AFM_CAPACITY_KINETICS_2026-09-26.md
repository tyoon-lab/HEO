# HEO Manuscript v7 — AFM-Target Capacity–Kinetics/GITT Diffusivity Audit Draft

**Date:** 2026-09-26  
**Status:** capacity–kinetics narrative retained; raw-GITT conventional diffusivity audit integrated into Figure 4 and Section 2.3  
...
The opposite HEO/BM-HEO rankings do not show that Li transport is absent. They show that a conventional apparent diffusion coefficient need not preserve the fast–slow ordering of the full kinetic response in a reconstructive conversion electrode. In the present data, the voltage terms entering the conventional GITT expression change unequally after milling, so the resulting $D_{\mathrm{app}}$ incorporates state-dependent thermodynamic and conversion contributions together with transport. The four materials therefore cannot be described as uniform acceleration or deceleration along one conversion-rate coordinate. Figure 5 next tests whether the anomalous relaxation is specifically localized to the conversion region rather than to a generic cell-relaxation process.
...
A conventional GITT diffusivity analysis provides an additional constraint. For the compositionally identical HEO/BM-HEO pair, the apparent diffusivity ranks BM-HEO as faster across the entire state-matched 200–800 mAh g⁻¹ interval, while the direct current-off relaxation is slower over nearly the same range. Apparent $D$ therefore does not preserve the kinetic ordering of the conversion-associated relaxation and should not be interpreted as a direct measure of overall conversion kinetics.

A literature-informed microkinetic model recovers the conventional homogeneous limit in which faster kinetics increases cutoff-limited capacity and accelerates relaxation, but also shows that multistep conversion can produce higher accessible capacity together with slower relaxation when reaction accessibility and internal timescales change together. Capacity and relaxation are therefore kinetically linked but not kinetically equivalent. Improved capacity in a conversion electrode should not automatically be interpreted as uniform kinetic acceleration, and a single apparent transport parameter cannot by itself quantify the full conversion-kinetic response.
