# HEO Manuscript v8 — AFM-Target Conversion Kinetics / GITT Ranking Draft
...
Under finite-rate operation, faster reaction kinetics are generally expected to increase the accessible capacity of conversion-type anodes. Conversion, however, proceeds through multiple coupled processes, including electron transfer, structural reconstruction, nucleation and phase growth, and cation/oxygen redistribution, raising the question of whether accessible capacity and kinetic response necessarily evolve together. Here, spinel high-entropy oxide (HEO) anodes are used as a model system to examine this relationship, with ball milling and Mg incorporation providing complementary perturbations that respectively enhance and suppress accessible conversion capacity. These perturbations reveal a mismatch between accessible capacity and the conversion-associated kinetic response. Ball milling increases accessible capacity despite slower effective GITT relaxation. More importantly, conventional GITT analysis gives the opposite kinetic ordering: the apparent diffusivity is higher for BM-HEO at all 37 state-matched points, whereas the direct current-off relaxation is slower at 35 of 37 points. Microkinetic analysis shows that such counterintuitive capacity–kinetics behavior can arise naturally from the multistep character of conversion, in which reaction accessibility and internal relaxation timescales need not change in parallel. These results show that apparent GITT diffusivity can fail to preserve the kinetic ordering of reconstructive conversion electrodes and should not be used as a standalone measure of overall conversion kinetics.
...
Under finite-rate operation, faster reaction kinetics are generally expected to reduce polarization and increase the fraction of thermodynamically available capacity reached before the voltage cutoff. Accessible capacity is therefore often interpreted together with kinetic descriptors when comparing electrode modifications. For a simple homogeneous reaction, the same acceleration should also shorten the relaxation after current interruption. Whether these observables preserve a common fast–slow ordering becomes less obvious when the electrode reaction itself is reconstructive.

Conversion reactions combine electron transfer and Li incorporation with metal–oxygen bond rearrangement, nucleation and phase growth, cation/oxygen redistribution, and product formation across heterogeneous regions of the electrode. These coupled processes can operate on different timescales and can respond differently to structural or compositional modification. A single material change may therefore alter the amount of reaction accessed under load, the magnitude of the nonequilibrium voltage response, and the subsequent relaxation timescale by different amounts.

Despite this complexity, conventional GITT-derived apparent Li-ion diffusion coefficients are widely used as kinetic comparators in conversion-type anodes, including HEOs.[6,33–36] In these studies, a larger apparent $D_{\mathrm{Li}}$ is commonly associated with faster ion transport and improved rate or capacity performance. The conventional reduction, however, compresses the pulse response into one transport parameter under assumptions that become restrictive in phase-transforming systems. Finite reaction kinetics can distort GITT-derived diffusion coefficients,[37] and phase-transformation, non-ideal thermodynamic, and pulse-model effects can require treatment beyond the standard single-$D$ expression.[15,21,24] Recent mechanistic work on spinel HEOs likewise distinguishes insertion-stage Li transport from conversion-stage reaction, interface, and oxygen-migration processes.[20] Whether an apparent GITT diffusivity actually preserves the fast–slow kinetic ordering of a reconstructive conversion electrode therefore requires independent experimental testing.

High-entropy oxides provide a useful material system for such a test. Spinel (FeCoNiCrMn)₃O₄ is a conversion-type HEO anode in which multiple redox-active cations share a common oxide structure.[1–4,6] Lithiation produces substantial reconstruction: metallic species emerge progressively,[5] while diffraction and microscopy studies report spinel-to-rock-salt-like evolution, Li₂O/metal formation, and oxygen/cation rearrangement.[7,20] Ball milling and Mg incorporation provide complementary perturbations of this conversion landscape. Milling increases surface area and has been associated with enhanced conversion reversibility,[6,12] whereas Mg-containing HEOs show greater structural retention but lower accessible capacity.[8–10] These contrasting changes allow accessible reaction extent and kinetic response to be varied without presuming that one is a proxy for the other.

Here, HEO, ball-milled HEO (BM-HEO), Mg-containing HEO (Mg-HEO), and BM-Mg-HEO are compared from conventional electrochemistry through state-resolved GITT relaxation. The primary test asks whether the higher accessible capacity produced by ball milling is accompanied by faster conversion-associated relaxation and whether the conventional GITT apparent diffusivity gives the same kinetic ranking. Differential-capacity analysis then localizes the anomalous response to the conversion region, cycle-resolved GITT examines its evolution with reaction history, and a coarse-grained microkinetic model tests whether the resulting capacity–kinetics mismatch is physically consistent with multistep conversion.
...
## 2.3. Higher capacity and apparent diffusivity do not imply faster conversion kinetics
...
A conventional GITT diffusivity analysis of the same HEO/BM-HEO pair gives the opposite kinetic ordering (Figure 4b). Because HEO and BM-HEO have the same nominal composition, the comparison is made as a state-matched relative apparent-diffusivity ranking rather than as an absolute $D$. Across 37 matched states spanning 200–800 mAh g⁻¹, $D_{\mathrm{app,BM}}/D_{\mathrm{app,HEO}}$ remains above unity at every state and has a median value of 1.78. In contrast, the direct relaxation-rate ratio, $t_{63,\mathrm{HEO}}/t_{63,\mathrm{BM}}$, has a median value of 0.762 and lies below unity at 35 of 37 states. The conventional diffusivity reduction would therefore classify BM-HEO as kinetically faster even though its current-off relaxation is slower.

This inversion is particularly consequential because larger GITT-derived diffusion coefficients are commonly used to support faster-kinetics interpretations in HEO and other conversion-anode studies.[6,33–36] If only the capacity and conventional $D_{\mathrm{app}}$ were considered here, BM-HEO would naturally be described as a faster electrode after milling. The state-matched relaxation gives the opposite result. The discrepancy is therefore not merely an uncertainty in the numerical value of $D$; the scalar apparent diffusivity fails to preserve the kinetic ordering obtained directly from the current-off response.
...
The opposite HEO/BM-HEO rankings do not show that Li transport is absent or that GITT-derived $D_{\mathrm{app}}$ has no operational value. They show that apparent diffusivity cannot be assumed to represent the overall conversion rate. In the present data, the voltage terms entering the conventional GITT expression change unequally after milling, so the resulting $D_{\mathrm{app}}$ combines transport sensitivity with state-dependent thermodynamic and conversion contributions. The full-pulse square-root-time audit in the Supporting Information provides an additional warning against forcing the reconstructive transient into one Fickian coordinate. The four materials therefore cannot be described as uniform acceleration or deceleration along one conversion-rate axis. Figure 5 next tests whether the anomalous relaxation is specifically localized to the conversion region rather than to a generic cell-relaxation process.
...
This heterogeneous calculation is not a fit to BM-HEO and does not identify a specific microscopic step altered by milling. Rather, it shows that the experimental trend is consistent with multistep conversion kinetics and cannot be represented by a uniform change in one rate constant. Ball milling is therefore interpreted as an accessibility-enhancing perturbation whose capacity increase does not imply uniform kinetic acceleration, while Mg suppresses accessible conversion without proportionally shifting the relaxation timescale. The model does not calculate a GITT diffusion coefficient; instead, it establishes the more general reason why a scalar descriptor derived from a composite voltage response need not preserve the kinetic ordering of a multistep network when accessible populations and internal timescales change together. The experimentally observed inversion between $D_{\mathrm{app}}$ and $t_{63}$ is consistent with this distinction. Capacity, apparent diffusivity, and GITT relaxation are all kinetically coupled to the conversion process, but they are not kinetically equivalent observables.
...
The conventional GITT analysis exposes a second and more general mismatch. For the compositionally identical HEO/BM-HEO pair, $D_{\mathrm{app}}$ ranks BM-HEO as faster at all 37 state-matched points between 200 and 800 mAh g⁻¹, whereas the direct current-off relaxation is slower at 35 of 37 points. A larger apparent diffusion coefficient therefore does not necessarily indicate faster overall conversion kinetics. This result does not exclude Li transport from the conversion response; it shows that the conventional apparent diffusivity can combine transport, thermodynamic, and transformation contributions in a way that changes the resulting kinetic ordering.

The microkinetic analysis provides a physical basis for this behavior. A homogeneous acceleration increases cutoff-limited capacity and shortens relaxation, whereas a multistep network can produce higher accessible capacity together with slower relaxation when reaction accessibility and internal timescales change together. Conversion kinetics therefore cannot, in general, be ranked by apparent GITT diffusivity alone. Apparent $D$ may remain useful as an operational transport descriptor, but independent kinetic observables are required before it is interpreted as a measure of the overall rate of a reconstructive conversion reaction.
