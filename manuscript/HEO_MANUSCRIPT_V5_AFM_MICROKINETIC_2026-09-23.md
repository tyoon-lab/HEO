...
Electrode performance is commonly discussed in terms of how much electrochemical reaction can be accessed and how rapidly that reaction proceeds. Under finite-rate operation, higher accessible capacity or improved rate capability is often associated with faster reaction kinetics because reduced kinetic polarization allows a larger fraction of the thermodynamically available reaction to be reached before the voltage cutoff. In a simple homogeneous reaction picture, the same kinetic processes that control reaction progress under current would also govern the return toward equilibrium after current interruption. Higher accessible capacity would therefore be expected, in general, to accompany faster relaxation. Whether this simple correspondence between capacity and kinetics remains valid for reconstructive conversion electrodes, however, is less clear.

Conversion reactions are inherently multistep and structurally heterogeneous. Electron transfer and Li incorporation are coupled to metal–oxygen bond rearrangement, nucleation and phase growth, cation/oxygen redistribution, and the formation of metallic and Li₂O-rich products, with different processes operating over different time and length scales. Consequently, finite-current capacity depends not only on intrinsic reaction rates but also on how much material can access the reaction network before cutoff, whereas current-off relaxation reflects how the nonequilibrium state populated during reaction redistributes toward equilibrium. The two observables are therefore kinetically linked through the same reaction network but need not be kinetically equivalent. This distinction should become particularly important when a materials modification changes reaction accessibility and the distribution of internal kinetic timescales at the same time.

High-entropy oxides (HEOs) provide a useful platform for testing this relationship. HEOs accommodate several principal cations within a common oxide structure, producing broad distributions of local coordination and redox environments that are difficult to realize in conventional single- or few-cation oxides.[1,2] Spinel (FeCoNiCrMn)₃O₄ is a representative conversion-type HEO anode for which single-phase spinel formation and high reversible capacity have been reported.[3,4,6] Its lithiation proceeds through substantial structural reconstruction rather than simple insertion into an intact host. Atomic-scale analysis has shown Mn nanocrystal formation already at 0.5 V and metallic Cr, Fe, Ni, and Co at deeper lithiation,[5] while complementary diffraction and microscopy studies have resolved spinel-to-rock-salt-like evolution, Li₂O/metal formation, and oxygen/cation rearrangement during conversion.[7,20] The multication and reconstructive nature of this chemistry therefore provides a natural setting in which reaction accessibility and kinetic relaxation may respond differently to materials modification.

Ball milling and Mg incorporation offer complementary perturbations of this conversion landscape. For the same nominal (FeCoNiCrMn)₃O₄ composition, mechanical processing and particle fragmentation have been reported to increase conversion reversibility and interfacial storage,[6,12] while high-energy milling can alter the spinel/rock-salt balance in related spinel HEOs.[11] Mg-containing conversion HEOs show a different response: electrochemically inactive or weakly active Mg-derived components can stabilize oxide-derived structures,[8–10] and operando imaging has associated higher Mg content with greater structural retention but lower accessible capacity.[9] These contrasting modifications make it possible to perturb accessible reaction extent and structural stability without assuming that all kinetic processes change in the same direction. GITT is well suited to examine the resulting kinetic response. Although GITT in HEO anodes has largely been used to estimate an apparent Li-ion diffusion coefficient,[6] studies of phase-transforming electrodes show that the pulse and relaxation response also contains information on state-dependent kinetics, phase transformation, and structural relaxation beyond a single diffusivity.[21–24,26] GITT relaxation can therefore be used here as a state-resolved probe of conversion-associated kinetics.

Here, spinel Fe–Co–Ni–Cr–Mn HEO, ball-milled HEO (BM-HEO), Mg-containing HEO (Mg-HEO), and BM-Mg-HEO are compared to determine how accessible capacity and conversion kinetics are related. Conventional electrochemical measurements first establish that ball milling increases accessible capacity whereas Mg incorporation suppresses it. GITT relaxation then reveals that these capacity changes do not follow a single fast–slow kinetic trend: ball milling accesses more reaction while the effective relaxation becomes slower, whereas Mg suppresses accessible conversion without a proportional change in relaxation timescale. Differential-capacity analysis localizes this mismatch to the conversion region, and cycle-resolved GITT shows that the conversion-associated kinetic response evolves strongly with reaction history. A literature-informed microkinetic model finally shows why this behavior is physically possible: the model recovers the expected capacity–relaxation coupling for a homogeneous reaction, but a heterogeneous multistep conversion network can produce higher accessible capacity together with slower relaxation when reaction accessibility and internal kinetic timescales are modified simultaneously. The results therefore show that capacity and relaxation are kinetically linked but cannot be reduced to a single fast–slow descriptor in multicomponent conversion electrodes.
...
Because capacity is also kinetic-dependent, the model was next tested under galvanostatic discharge to a fixed voltage cutoff. For a single homogeneous population, scaling all rate constants together gives the conventional behavior: increasing the global rate scale from 0.5 to 2.0 raises the normalized cutoff capacity from 0.299 to 0.832 while shortening matched-state \(t_{63}\) from 22.45 to 6.95 min. Thus, the experimental capacity–relaxation mismatch cannot be interpreted as an absence of kinetic coupling.

A minimal heterogeneous extension shows how the opposite experimental trend can nevertheless occur. Retaining the original fast population and adding an additional accessible population with a ten-times-slower reconstruction step increases the illustrative cutoff capacity by 17.7% while increasing matched-state \(t_{63}\) by 13.6%. This calculation is an existence proof rather than a fit to BM-HEO, but it demonstrates that a materials modification can increase the amount of reaction accessible before cutoff while simultaneously increasing the weight of slower internal relaxation. Capacity and post-interruption relaxation are therefore both governed by conversion kinetics, yet they need not vary monotonically with one another in a heterogeneous multistep network.

This interpretation resolves the experimental trends without mapping ball milling or Mg incorporation uniquely onto individual microscopic rate constants. Ball milling increases the amount of material and interface that can access the conversion network, consistent with its larger surface area and higher reversible capacity; the present data additionally show that this accessibility increase is not equivalent to a uniform acceleration of all kinetic steps. Mg incorporation instead suppresses accessible deep conversion and the first-cycle relaxation amplitude without proportionally changing the later-cycle relaxation timescale.
...
A minimal multi-step microkinetic model provides a physically consistent explanation. The model first recovers the expected homogeneous coupling in which globally faster kinetics increases cutoff-limited capacity and shortens relaxation. It then shows that higher capacity can coexist with longer relaxation when a modification increases accessible reaction population while also changing the distribution of internal kinetic timescales. This heterogeneous case is an existence proof rather than a microscopic assignment to ball milling. The results therefore show that capacity and post-interruption relaxation are kinetically coupled but cannot be reduced to a single fast–slow descriptor.
...
**Figure 7. Literature-informed microkinetic interpretation of the capacity–relaxation mismatch.** (a) Coarse-grained conversion network, \(O \rightleftharpoons I \rightleftharpoons I^* \rightleftharpoons C\), containing two reversible Faradaic steps separated by a structural reconstruction/conversion-activation step. The states are effective kinetic states rather than uniquely assigned phases. (b) Current-on and current-off balances. At open circuit, \(j_{\mathrm{ext}}=F(\nu_1r_1+\nu_3r_3)=0\) constrains the sum of partial Faradaic currents but does not require each partial rate to vanish; internal conversion-state redistribution can persist at conserved overall lithiation while the voltage relaxes. (c) Homogeneous kinetic-speed control. Scaling all rates together increases cutoff-limited capacity while shortening matched-state \(t_{63}\), demonstrating that capacity remains kinetic-dependent. (d) Heterogeneous-accessibility existence proof. Retaining the original fast population while adding an accessible slower-reconstructing population produces both higher cutoff capacity and longer matched-state \(t_{63}\), showing how the direction observed after ball milling is physically possible without assigning a unique microscopic ball-milling mechanism. The local relation \(E(t)-E_{\mathrm{eq}}=\sum_i B_i\exp(-t/\tau_i)\) provides the corresponding mode-level interpretation.
