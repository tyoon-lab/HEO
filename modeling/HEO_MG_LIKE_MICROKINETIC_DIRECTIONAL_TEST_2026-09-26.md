...
The calculation is now reproduced directly by
`modeling/heo_capacity_relaxation_heterogeneous_validation.py`, using the same rate equations and parameter set as the current Figure 7 validation.
...
- current-off relaxation is evaluated from the common initial state at the same matched normalized passed charge, \(\Delta Q=0.30\), corresponding to a 1500 s pulse at the fixed normalized current;
...
- \(Q_{\rm cutoff}\): **−29.6%**;
- \(\Delta E_{\rm relax}\): **−13.3%**;
- \(t_{63}\): **+21.8%**.

These values replace the earlier provisional table, which was not generated by a committed reproducible matched-state code path. The directional conclusion is unchanged.
